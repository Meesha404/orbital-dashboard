...
from data.tle_fetcher import fetch_starlink_tles, parse_tles, load_fallback_tles
...
/* Exclude Streamlit's internal icon-font elements (e.g. the sidebar
   collapse arrow) from the override above — they rely on a ligature
   font where text like "keyboard_double_arrow_left" renders as a
   glyph, not literal text. The wildcard [class*="css"] above is broad
   enough to accidentally capture these, so restore their font here. */
...
def load_satellite_data(n_sats: int):
    """
    Load Starlink TLEs from live CelesTrak data.

    Returns (satellites, is_live). If the live fetch fails for any reason
    (timeout, CelesTrak rate-limit, network block on the hosting platform),
    falls back to a bundled, clearly-labeled synthetic constellation instead
    of crashing the whole app — a demo link that dies for anyone who wasn't
    there to click "retry" is worse than one running on cached data.
    """
    try:
        raw_tles = fetch_starlink_tles()
        satellites = parse_tles(raw_tles)
        return satellites[:n_sats], True
    except Exception as e:
        logging.getLogger(__name__).warning(f"Live CelesTrak fetch failed, using fallback data: {e}")
        raw_tles = load_fallback_tles()
        satellites = parse_tles(raw_tles)
        return satellites[:n_sats], False

with st.spinner("INITIALIZING ORBITAL TELEMETRY..."):
    satellites, data_is_live = load_satellite_data(max_sats)

if not data_is_live:
    st.warning(
        "⚠ LIVE CELESTRAK FETCH FAILED — showing a bundled offline demo "
        "constellation instead (same orbital shell, synthetic satellites, "
        "not real Starlink positions). This usually means the hosting "
        "platform's outbound connection to celestrak.org is blocked or "
        "timed out, not that anything in the app is broken. "
        "Click REFRESH TLE DATA in the sidebar to retry the live source."
    )
...
<div class='orbital-header'>
    <div style='display:flex; align-items:center; justify-content:space-between;'>
        ...
        <div style='text-align:right;'>
            <div class='live-badge'>{'● LIVE' if data_is_live else '◐ DEMO DATA'}</div>
